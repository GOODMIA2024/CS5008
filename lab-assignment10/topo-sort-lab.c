...
void topologicalOrder(int arr[][MAX], Queue *t, int n)
{
    ...
    // Step 1: Compute in-degree(N) of every node in the graph
    for(i=1;i<=n;i++)
    {
        indeg[i] = find_indegree(arr,i,n);
        if(indeg[i] == 0) {
            enqueue(t, i); // Step 2: Add all the vertices with in-degree 0 to queue
        }
    }
    
    
    // Step 3: Perform Dequeue operation until the queue is empty
    while(!isEmpty(t->front)) {
        delNode = dequeue(t);
        result[j++] = delNode; // Add the dequeued node to the result
    ...
    // Step 4: Print the topological ordering
